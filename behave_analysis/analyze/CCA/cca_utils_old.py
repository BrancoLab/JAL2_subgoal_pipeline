...
def preprocess_behavioral_data(df, columns_to_keep):
    ...
    for col in columns_to_keep:
        if col in ["mouse_x_position", "mouse_y_position", "distance_to_shelter", "distance_to_barrier1", "distance_to_barrier2"]:
            Y = np.append(Y, df[col].to_numpy()[:, np.newaxis], axis=1)
            name = np.append(name, col)
        elif col == "acceleration":
            # acc = savgol_filter(df["speed"].to_numpy(), window_length=15, polyorder=2, deriv=1, delta=dt)
            acc = np.gradient(df["speed"].to_numpy(), dt)
            Y = np.append(Y, acc[:, np.newaxis], axis=1)
            name = np.append(name, col)
        elif col == "hdir_velocity":
            # hv = savgol_filter(np.unwrap(df["hdir"].to_numpy()), window_length=5, polyorder=2, deriv=1, delta=dt)
            hv = np.gradient(np.unwrap(df["hdir"].to_numpy()), dt)
            Y = np.append(Y, hv[:, np.newaxis], axis=1)
            name = np.append(name, col)
        elif col == "speed":
            Y = np.append(Y, np.log1p(df[col].to_numpy())[:, np.newaxis], axis=1)
            name = np.append(name, col)
        elif col in ["hdir", "hsa", "h_preflipbar_a", "h_postflipbar_a"]:
            Y = np.append(Y, np.cos(np.deg2rad(df[col].to_numpy()))[:, np.newaxis], axis=1)
            Y = np.append(Y, np.sin(np.deg2rad(df[col].to_numpy()))[:, np.newaxis], axis=1)
            name = np.append(name, col + "_cos")
            name = np.append(name, col + "_sin")
    Y = Y[:, 1:]  # remove the initial empty column
    name = name[1:]  # remove the initial empty column
    return Y, name


def select_train_test_frames(video_df, condition, method):
    ...
    frames_by_condition_explore = filter_video_dataframe(video_df, condition=condition, outofshelter=True, exclude_escape=True, exclude_homings=True)["frames"].to_numpy() - 1
    homing_indices = (
        filter_video_dataframe(video_df, condition=condition, outofshelter=True, exclude_escape=False, select_homings=True, select_escape=True)["frames"].to_numpy() - 1
    ...
    if "random split" in method:
        ...
        if method == "random split":
            ...
            n_items = len(homing_indices)

        explore_train_idx = np.random.choice(explore_indices, size=n_items, replace=False)
        explore_test_idx = np.array([idx for idx in explore_indices if idx not in explore_train_idx])

    elif method == "half":
        explore_indices = frames_by_condition_explore

        explore_train_idx = explore_indices[: len(explore_indices) // 2]
        explore_test_idx = explore_indices[len(explore_indices) // 2 :]

    elif "balanced_bins" in method:
        ...
        cum_bins_explore = cum_bins[frames_by_condition_explore]
        cum_bins_homing = cum_bins[homing_indices]
        # 1. Get counts for Homing (your template)
        bin_id_homing, counts_homing = np.unique(cum_bins_homing, return_counts=True)

        # 2. Find how many of those same bins exist in Explore
        counts_explore_in_homing_bins = []
        for b_id in bin_id_homing:
            count = np.sum(cum_bins_explore == b_id)
            counts_explore_in_homing_bins.append(count)

        counts_explore_in_homing_bins = np.array(counts_explore_in_homing_bins)
        ...
        ratios = np.where(counts_explore_in_homing_bins > 0, counts_explore_in_homing_bins / counts_homing, 1.0 / counts_homing)

        scaling_factor = max(np.min(ratios), 1)

        # 4. Apply scaling and Subsample
        matched_explore_indices = []

        for b_id, h_count, e_available in zip(bin_id_homing, counts_homing, counts_explore_in_homing_bins):
            ...
            if n_to_draw > 0:
                in_bin_mask = cum_bins_explore == b_id
                available_indices = frames_by_condition_explore[in_bin_mask]

                selected = np.random.choice(available_indices, n_to_draw, replace=False)
                matched_explore_indices.extend(selected)

        explore_test_idx = np.sort(matched_explore_indices)
        explore_train_idx = [x for x in frames_by_condition_explore if x not in matched_explore_indices]

    return explore_train_idx, explore_test_idx, homing_indices
