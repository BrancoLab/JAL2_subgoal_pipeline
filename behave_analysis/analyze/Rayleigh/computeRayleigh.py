...
def compute_all_clusters_rayleigh(aefizz, all_angles: list) -> None:
...
def single_cluster_plots(aefizz, all_angles, all_conditions, data_path, plot_save_path):
...
def rayleigh_vector(
    ...
    bin_angles, bin_angle_center = generate_bins(number_of_bins=aefizz.settings.number_of_bins, start=-np.pi, stop=np.pi)
    ...
    cluster_Ids = aefizz.cluster_Ids[aefizz.cluster_Ids > 0]
    ...
    for count in tqdm(np.arange(len(cluster_Ids)), desc=f"Running Rayleigh on cluster out of  {len(cluster_Ids)}"):
        ...
        if aefizz.settings.rayleigh_significance == "bootstrap":
        ...
        elif aefizz.settings.linear_shift:
        ...
        for c_count, comp in enumerate(np.unique(compartment)):
            ...
            Rayleigh[count, c_count], Rayleigh_theta[count, c_count], angle_firing_hist[count, :, c_count] = compute_rayleigh_cluster(
                X[compartment == comp, count], binned_angles[compartment == comp], nbins=aefizz.settings.number_of_bins, return_all_stats=True
            )
            ...
            elif aefizz.settings.rayleigh_significance == "bootstrap":
...
def all_clusters_polar_plots(rayleigh_results, save_path, show_plots):
    ...
    aefizz.tuning_dict['angles'] is a binned set of angles and aefizz.tuning_dict[title][0] will give you the firing rates for cluster 0.
...
def identify_which_compartment(aefizz, filtered_video_df: pl.DataFrame) -> np.ndarray:
