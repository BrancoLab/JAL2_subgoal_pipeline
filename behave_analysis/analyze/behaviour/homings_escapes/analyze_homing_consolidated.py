...
class HomingAnalyzer:
        ...
        for exp in self.experiments:
            ...
            try:
                ...
                self.session_data[session_name] = SessionData(
                    ...
                    xy_position=np.column_stack([tracking_data["avg_loc"][:, 0], tracking_data["avg_loc"][:, 1]]),
                    is_homing=ishoming,
    ...
    def label_extracted_runs(self, overlap_threshold: float = 0.5) -> None:
        ...
        for run in self.extracted_runs:
            ...
            for manual_run in session_manual:
                ov = self._interval_overlap(run.onset_frame, run.offset_frame, manual_run.onset_frame, manual_run.offset_frame)
    ...
    def compute_feature_distributions(self) -> Dict:
        ...
        for fname in feature_names:
            ...
            for s_name in self.session_data.keys():
                s_exp = np.array([getattr(r, fname) for r in self.extracted_runs if r.session_name == s_name and r.is_exploration], dtype=float)
                s_tgt = np.array([getattr(r, fname) for r in self.manual_runs if r.session_name == s_name], dtype=float)
        ...
        for idx, (fname, stats_dict) in enumerate(self.feature_ranking):
            logger.info(f"    {idx+1}. {fname}: d={stats_dict['cohens_d']:.3f}, " f"AUC={stats_dict['auc']:.3f}, p={stats_dict['pval']:.3e}")
    ...
    def plot_feature_distributions(self, only_top_n: int = 6, n_bins: int = 20) -> None:
        ...
        for idx, (fname, _) in enumerate(self.feature_ranking[:n_plots]):
            ...
            pooled = np.array([getattr(r, fname) for r in (self.extracted_runs + self.manual_runs) if np.isfinite(getattr(r, fname, np.nan))], dtype=float)
            if pooled.size == 0:
                ax.axis("off")
                continue
            ...
            for session_name in session_names:
                session_explore = [r for r in self.extracted_runs if r.session_name == session_name and r.is_exploration]
                session_target = [r for r in self.manual_runs if r.session_name == session_name]
    ...
    def fit_classification_gates(self, target_recall: float = 0.90) -> Dict[str, Dict]:
        ...
        for fname, _ in self.feature_ranking:
            ...
            if best_gate is not None:
                ...
                logger.info(
                    f"  {fname}: keep if value {best_gate['dir']} {best_gate['threshold']:.4f} " f"(recall={best_gate['recall']:.3f}, precision={best_gate['precision']:.3f})"
                )
    ...
    def compute_manualvsauto_overlap(
        ...
        if candidates_by_session is None:
            if hasattr(self, "candidates_by_session"):
                ...
                logger.error("Session-wise candidates are required for accurate Phase 3 overlap. " "Run run_phase2_classification() first or pass candidates_by_session.")
                return {}

        if candidates is not None and candidates_by_session is not None and hasattr(self, "phase2_candidates_by_session"):
            logger.warning("Ignoring flat candidates list in Phase 3 because session-wise candidates are available.")
    ...
    def create_syd_viewer(
        ...
        def plot(state):
            ...
            Arena(ax=axs[0, 0], condition=condition, barrier_coordinates=data.barrier_loc[:-1], shelter_coordinates=data.shelter_loc, full_image=False)

            axs[0, 1].scatter(x[man_win], y[man_win], s=8, color="tab:orange", alpha=0.8, label="manual")
            axs[0, 1].plot(x, y, color="0.85", lw=1)
            Arena(ax=axs[0, 1], condition=condition, barrier_coordinates=data.barrier_loc[:-1], shelter_coordinates=data.shelter_loc, full_image=False)
