...
class PreprocessSingleTrialRegression:
    ...
    def __init__(
        ...
        self.homing_list, homing_df_s1, self.condition_per_homing = self.preprocess_homing_data(video_df=video_df)
        ...
        if condition:
            self.homing_data_single_dataframe = self.filter_by_condition(condition, self.condition_per_homing, self.homing_data_single_dataframe)
    ...
    def preprocess_homing_data(self, video_df) -> tuple:
        ...
        Returns:
            ...
            condition_per_homings (list): A list of conditions for each homing period
        """
        ...
        cocatenated_homing_data = self.concatenate_the_homing_data_into_a_single_df(homing_info)
    ...
    def add_dependent_index_variable_to_homing_info(self, homing_data_single_dataframe: pl.DataFrame):
        ...
        compute_index = Index()
        post_flip_index = compute_index(hsa, post_flip_goal_a)
        pre_flip_index = compute_index(hsa, pre_flip_goal_a)

        # Add the index to the homing data
    ...
    def extract_data_from_homings(self, homing_object: dict, video_df: pl.DataFrame) -> list:
        ...
        Returns:
             homing_info (list) of homing dataframes for each homing period
             condition_per_homing (list) of conditions for each homing period"""
        homing_info = []
        condition_per_homing = []
        for onset, offset in zip(homing_object.onset_frames, homing_object.offset_frames):
            homing = video_df[int(onset) - 1 : int(offset)]  # Frames are 1 indexed
    ...
    def concatenate_the_homing_data_into_a_single_df(self, homing_info: list) -> pl.DataFrame:
        """Take each df element of the list and concatenate them into a single dataframe"""
    ...
    def filter_by_condition(self, condition: str, condition_per_homing: list, dataframe: pl.DataFrame) -> pl.DataFrame:
        """Filter the homing data by a condition"""
        mapping = {k: v for k, v in zip(np.unique(dataframe["homing_id"]), condition_per_homing)}  # Assign a condition to each homing id
        homings_ids_in_condition = [homing_id for homing_id in np.unique(dataframe["homing_id"]) if mapping[homing_id] == condition]
        ...
        dataframe = dataframe.filter(pl.col("homing_id").is_in(homings_ids_in_condition))
        return dataframe
...
class Index:
    """Class for computing the index between two angles"""

    def __call__(self, angle1, angle2):
        return self.compute_index(angle1, angle2)
