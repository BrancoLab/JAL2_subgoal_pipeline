...
class TunEdModel:
    """Uses conditional indepedence tests to determine whether a neuron is tuned to a stimulus variable or not."""
    def __init__(self, 
                 post_process_object, 
    ...
    def execute_model_per_cluster(self, nbins4sig) -> dict:
        ...
        for cluster in np.unique(self.post_process_object.spike_data["spike_clusters"]):
            ...
            for conIdx, condition in enumerate(self.conditions):                
                ...
                hdir_tuning_object, hsa_tuning_object = self.compute_mean_firing_curves(raster, hdir, hsa, Nbins, Nsamples)
                ...
                self.plot_tuning_functions(len(self.conditions),
                                            conIdx,
                                            hdir_tuning_object, 
                                            hsa_NH_object, 
                                            hsa_tuning_object, 
    ...
    def plot_tuning_functions(self,
        ...
        col0.plot(hdir_tuning_object.bin_centres, hdir_tuning_object.tuning_func[0, :], '.-', label='Tuning to hdir', color="cornflowerblue")
        col0.fill_between(hdir_tuning_object.bin_centres, hdir_tuning_object.tuning_func[0, :] - hdirCI["observedCI"][0], 
                           hdir_tuning_object.tuning_func[0, :] + hdirCI["observedCI"][0], alpha=0.1, color="cornflowerblue")
        col0.plot(hdir_tuning_object.bin_centres, hsa_NH_object.tuning_func_nh[0, :], '.--', label='Tuning to hdir given NH that driver is hsa', color='darkorchid')
        col0.fill_between(hdir_tuning_object.bin_centres, hsa_NH_object.tuning_func_nh[0, :] - hsaCI["expectedCI"][0], 
        ...
        col1.plot(hsa_tuning_object.bin_centres, hsa_tuning_object.tuning_func[0, :], '.-', label='Tuning to hsa', color='cornflowerblue')
        col1.fill_between(hsa_tuning_object.bin_centres, hsa_tuning_object.tuning_func[0, :] -  hsaCI["observedCI"][0], 
                           hsa_tuning_object.tuning_func[0, :] + hsaCI["observedCI"][0], color='cornflowerblue', alpha=0.1)
        col1.plot(hsa_tuning_object.bin_centres, hdir_NH_object.tuning_func_nh[0, :], '.--', label='Tuning to hsa given NH that driver is hdir', color='darkorchid')
        col1.fill_between(hsa_tuning_object.bin_centres, hdir_NH_object.tuning_func_nh[0, :] - hdirCI["expectedCI"][0], 
