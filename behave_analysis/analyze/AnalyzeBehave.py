...
class AnalyzeBehave:
    ...
    def __init__(self, session):
        ...
        self.tracking_data = open_tracking_data(self)
        ...
        self.spatial_efficiency(self.settings.stim_type)

    def spatial_efficiency(self, stim_type):
        """Plot escape trajectories as well as optimal path"""
        plt.figure(figsize=(25, 16))
        plt.subplots_adjust(hspace=0.3)
        ntrial = len(getattr(self.session, stim_type).onset_frames)
        nrows = 3
        ncols = ntrial // nrows + (ntrial % nrows > 0)

        # plot trajectory
        self.trajectory_length = np.empty(len(getattr(self.session, stim_type).onset_frames))
        self.optimal_trajectory_length = np.empty(len(getattr(self.session, stim_type).onset_frames))
        self.spatial_efficiency_value = np.empty(len(getattr(self.session, stim_type).onset_frames))
        zippy = zip(getattr(self.session, stim_type).onset_frames, getattr(self.session, stim_type).stimulus_durations)

        for trial_num, (onset_frames, stimulus_durations) in enumerate(zippy):
            ...
            self.trajectory_length[trial_num] = self.plot_escape_trajectories(
                onset_frames[0], stimulus_durations[0], ax
            )
            self.optimal_trajectory_length[trial_num] = self.plot_optimal_trajectories(onset_frames[0], ax, condition)
            self.spatial_efficiency_value[trial_num] = (
                self.optimal_trajectory_length[trial_num] / self.trajectory_length[trial_num]
            )
            ax.set_title(f"spatial efficiency = {round(self.spatial_efficiency_value[trial_num], 2)}")
    ...
    def plot_escape_trajectories(self, onset_frames, stimulus_durations, ax):
        ...
        for i, stim_status in enumerate(np.arange(0, stimulus_durations, 0.025)):
            trail_color[i, :] = get_color_based_on_speed(
                speed=speed[i], object_to_color="trail", stim_status=stim_status, stim_type=self.settings.stim_type
            )
            if i > 0:
                distance_travelled = np.append(
                    distance_travelled, np.sqrt((x_loc[i] - x_loc[i - 1]) ** 2 + (y_loc[i] - y_loc[i - 1]) ** 2)
                )
    ...
    def plot_optimal_trajectories(self, onset_frames, ax, condition):
        ...
        if not (condition == "shelter_only"):
            if condition == "barrier_present":  # double sided barrier
                nearest_barrier_edge = np.argmin(
                    [
                        np.sqrt(
                            (x_loc - self.tracking_data["barrier_loc"][0][0]) ** 2
                            + (y_loc - self.tracking_data["barrier_loc"][0][1]) ** 2
                        ),
                        np.sqrt(
                            (x_loc - self.tracking_data["barrier_loc"][1][0]) ** 2
            ...
            trjectory_to_barrier = np.sqrt(
                (x_loc - self.tracking_data["barrier_loc"][nearest_barrier_edge][0]) ** 2
        ...
        ax.plot(
            [x_loc, np.mean([self.tracking_data["shelter_loc"][0][0], self.tracking_data["shelter_loc"][1][0]])],
            [y_loc, np.mean([self.tracking_data["shelter_loc"][0][1], self.tracking_data["shelter_loc"][1][1]])],
            color=c_line,
        )
        trajectory_to_shelter = np.sqrt(
            (np.mean([self.tracking_data["shelter_loc"][0][0], self.tracking_data["shelter_loc"][1][0]]) - x_loc) ** 2
...
def base_plotting(ax, tracking, condition):
    ...
    if not (condition == "shelter_only"):
        ...
        plt.plot(
            [bar_loc[0], bar_loc[1]], [tracking["barrier_loc"][0][1], tracking["barrier_loc"][1][1]], color=[0, 0, 0]
        )
