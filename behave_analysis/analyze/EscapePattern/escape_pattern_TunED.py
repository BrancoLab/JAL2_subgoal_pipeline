...
def escape_pattern_TunED(aefizz, variable):
    ...
    """2. Compute TunED"""
    tuned_settings = {
        "bin_edges": np.arange(aefizz.settings.ep_bins + 1),
    ...
    for c in range(n_cond):  # which condition do we want to look at
        ...
        if tuned_settings["stats"] == "bootstrap":
            ...
            for i, v in enumerate(tqdm(resampled_vector, total=tuned_settings["stats_samples"], desc=f"TunED bootstrap resamples for condition {c}", leave=False)):
                ...
                mu_v1_bs = compute_avg_firing_tuning_curve(escape_matrix_bs, v1_bs, aefizz.settings.ep_bins, trial_start=cond_start)
                mu_v2_bs = compute_avg_firing_tuning_curve(escape_matrix_bs, v2_bs, aefizz.settings.ep_bins, trial_start=cond_start)
