...
class Visualize_efizz:
    ...
    def PSTH_all_neurons(self, stim_type):
        ...
        for trial_num, onset_frames in enumerate(self.processed_data.Visualize.session[__dict__[stim_type]]["onset_frames"]):
            ...
            spikes_trial = self.processed_data.spike_data.filter(
                (self.processed_data.spike_data["aligned_spike_times"] > time1) & (self.processed_data.spike_data["aligned_spike_times"] < time2)
            )
    ...
    def PSTH_single_neurons(self, stim_type):
        ...
        for trial, onset_frames in enumerate(self.processed_data.Visualize.session[__dict__[stim_type]]["onset_frames"]):
            ...
            filt = filt.select(
                [
                    pl.col("aligned_spike_times").apply(lambda x: x - (onset_frames / self.processed_data.Visualize.session["video"]["fps"])),
            ...
            if trial == 0:
                ...
                spikes_trial = spikes_trial.vstack(filt)
    ...
    def rasters(self, stim_type):
        ...
        for trial_num, (onset_frames, stim_duration) in enumerate(
            zip(self.processed_data.Visualize.session[__dict__[stim_type]]["onset_frames"], self.processed_data.Visualize.session[__dict__[stim_type]]["stimulus_durations"])
        ):
            ...
            ax.scatter(
                spikes_trial["aligned_spike_times"].to_numpy() - (onset_frames / self.processed_data.Visualize.session["video"]["fps"]),
            ...
            for rows in range(num_rows):
                for columns in range(num_cols):
                    if plot_counter < number_of_plots:
                        cluster = number_of_clusters[plot_counter]
                        spikes_trial_cluster = spikes_trial.filter(spikes_trial["spike_clusters"] == cluster)
                        axes[rows, columns].scatter(spikes_trial_cluster["aligned_spike_times"].to_numpy(), spikes_trial_cluster["trial"].to_numpy(), marker="|", s=10, c="k")
                        axes[rows, columns].set_title(f"Cluster: {cluster}")
    ...
    def compute_all_tunings_for_each_cell(self, compute_bootstrap=False):
        ...
        run = {
            ...
            "hsa": True,
            "pre_hsa": True,
            "h_postflipbar_a": True,
            "pre_h_postflipbar_a": True,
            "h_preflipbar_a": True,
            "pre_h_preflipbar_a": True,
            "h_bar_centre_a": False,
            "pre_h_bar_centre_a": False,
        }
    ...
    def compute_a_single_tuning_for_all_cells(self, which_angle, compute_bootstrap=False, object_present=True):
        ...
        folder_name = str(title) + "_" + self.processed_data.select_clusters + "_cluster_tuning_plots"
        plot_save_path = os.path.join(self.processed_data.Visualize.session["processed_path"], folder_name)
        ...
        self.all_clusters_polar_plots(title, plot_save_path)
    ...
    def rayleigh_vector(self, filtered_video_df, angle_filt, title, compute_bootstrap=False):
        ...
        for counter, c in enumerate(number_of_clusters):
            ...
            spike_to_video_df = spike_to_video_df.with_columns(
                spike_to_video_df[angle_filt].cut(bins=bin_angles, labels=[str(x) for x in bin_angle_center])["category"].alias("binned_angles")
            )
            ...
            if compute_bootstrap:
                ...
                for it in np.arange(len(shift_dist)):
                    ...
                    spike_to_video_df = spike_to_video_df.with_columns(
                        spike_to_video_df["rolled_angles"].cut(bins=bin_angles, labels=[str(x) for x in bin_angle_center])["category"].alias("binned_angles")
                    )
                    spike_to_video_df = spike_to_video_df.fill_null(strategy="zero")
        ...
        plt.savefig(str(self.rayleigh_path) + "/" + str(title) + "_" + self.processed_data.select_clusters + "_Rayleigh_vector_hist.png")
        if self.processed_data.Visualize.settings.show_plots:
            plt.show()
    ...
    def all_clusters_polar_plots(self, title, save_path):
        """
        ...
        for counter, c in enumerate(number_of_clusters):
            ...
            if self.tuning_dict["angles"].size > 0 and len(self.tuning_dict[title][counter, :]) > 0:
                ax.bar(
                    self.tuning_dict["angles"], self.tuning_dict[title][counter, :], width=(2 * np.pi) / (len(self.tuning_dict["angles"]) + 1), bottom=0.0, color="green", alpha=0.5
                )
            ...
            if self.Rayleigh_sig[title][counter] == 1:
                ax.title.set_text(
                    str(this_cluster["cluster_group"].to_numpy()) + " clu " + str(c) + " (sig.)" + "\n" + "Rayleigh = " + str(np.around(self.Rayleigh[title][counter], 2))
                )
            ...
            if np.logical_or(counter - (nrows * ncols * (fnum - 1)) == (ncols * nrows) - 1, counter == len(number_of_clusters) - 1):
                plt.tight_layout()
                plt.savefig(str(save_path) + "/" + str(title) + "_" + self.processed_data.select_clusters + "_cluster_polar_plots_" + str(fnum) + ".png")
                if self.processed_data.Visualize.settings.show_plots:
                    plt.show()
    ...
    def single_cluster_polar_plots(self, cluster):
        ...
        for subp, angle in enumerate(tuning_angles):
            ...
            if str("pre_" + angle) in self.tuning_dict:
                ...
                if len(counter) > 0:
                    ax.bar(
                        self.tuning_dict["angles"],
                        self.tuning_dict[str("pre_" + angle)][counter, :][0],
                    ...
                    ax.vlines(
                        self.Rayleigh_theta[str("pre_" + angle)][counter][0],
                        0,
                        self.Rayleigh[str("pre_" + angle)][counter][0] * np.amax(self.tuning_dict[str("pre_" + angle)][counter, :][0]),
                        colors="red",
                    )
                    ax.title.set_text(
                        angle
                        + "\n"
                        + "preRayleigh = "
                        + str(np.around(self.Rayleigh[str("pre_" + angle)][counter][0], 2))
                        + ", sig = "
                        + str(np.around(self.Rayleigh_sig[str("pre_" + angle)][counter][0], 2))
                        + "\n"
                        + "Rayleigh = "
                        + str(np.around(self.Rayleigh[angle][counter][0], 2))
                        + ", sig = "
                        + str(np.around(self.Rayleigh_sig[angle][counter][0], 2))
                    )
            ...
            if angle in self.tuning_dict:
                ...
                if len(counter) > 0:
                    ax.bar(
                        self.tuning_dict["angles"],
                        self.tuning_dict[angle][counter, :][0],
                        width=(2 * np.pi) / (len(self.tuning_dict["angles"]) + 1),
                        bottom=0.0,
                        color="green",
                        alpha=0.5,
        ...
        cluster_path = os.path.join(self.processed_data.Visualize.session["processed_path"], str(self.processed_data.select_clusters + "_individual_cluster_plots"))
    ...
    def spatial_position_firing(self):
        ...
        for counter, cluster in enumerate(self.processed_data.spike_data["spike_clusters"].unique()):
            ...
            axs[counter - (nrows * ncols * fnum)].scatter(
                spike_to_video_df["mouse_x_position"].to_numpy(),
                spike_to_video_df["mouse_y_position"].to_numpy(),
                s=5,
                c=cc(spike_to_video_df["spike_count"].to_numpy() * 2),
                linewidths=0,
                marker=".",
            ...
            if np.logical_or(counter - (nrows * ncols * (fnum - 1)) == (ncols * nrows) - 1, counter == len(self.processed_data.spike_data["spike_clusters"].unique()) - 1):
                plt.tight_layout()
                plt.savefig(
                    str(self.processed_data.Visualize.session["processed_path"])
                    + "/"
                    + self.processed_data.select_clusters
                    + "_clusters_spatial_position_firing_"
                    + str(fnum)
                    + ".png"
                )
                if self.processed_data.Visualize.settings.show_plots:
...
def filter_video_dataframe(dataframe, which_angle, object_present):
    ...
    """
    if which_angle == "hsa":
        filtered_video_df = dataframe.filter((dataframe["OutofshelterIdx"] == True) & (dataframe["EscapePeriod"] == False) & (dataframe["shelter_only"] == object_present))
        angle_filt = which_angle

    elif "bar" in which_angle:
        filtered_video_df = dataframe.filter((dataframe["OutofshelterIdx"] == True) & (dataframe["EscapePeriod"] == False) & (dataframe["barrier_present"] == object_present))
        angle_filt = which_angle
        ...
        filtered_video_df = dataframe.filter((dataframe["OutofshelterIdx"] == True) & (dataframe["EscapePeriod"] == False))
        angle_filt = which_angle
