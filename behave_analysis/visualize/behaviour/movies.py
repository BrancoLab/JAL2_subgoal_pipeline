...
from behave_analysis.track.register import (
    load_fisheye_correction_map,
    correct_and_register_frame,
)
    ...
    for trial_num, (onset_frames, stimulus_duration) in enumerate(metadata):
        ...
        for i in frames_in_this_trial:
            ...
            (
                ...
                head_loc,
                hdir,
            ) = get_current_position_and_speed(
                tracking_data, settings, frame_num
            )  #
            display_stimulus(actual_frame, stim_status, settings, stim_type, avg_loc, i)
            trail, trail_colors, trail_thicknesses = display_trail(
                settings,
                actual_frame,
                trail,
                trail_colors,
...
def display_stimulus(actual_frame, stim_status, settings, stim_type, avg_loc, i: int) -> None:
    ...
    if settings.display_stimulus and stim_status[i] == 0 and (stim_type == "audio" or (stim_type == "laser" and settings.display_tracking)):
        if stim_type == "laser":
...
def display_trail(
    ...
    if settings.display_trail:
        trail, trail_colors, trail_thicknesses = get_new_trail_segment(
            avg_loc,
            stim_type,
            speed,
            num_frames_past_stim,
...
def display_speed_on_frame(actual_frame, speed, stim_type):
    ...
    speed_text_color = get_color_based_on_speed(speed=speed, object_to_color="text", stim_status=None, stim_type=stim_type)
...
def display_heading_dir_on_frame(actual_frame, hdir, head_loc, bod_shelt_dir, hdir_shelt):
    ...
    heading_dir_x = int(magnitudeOfVector * np.cos(hdir))  # self.body_dir  # CPonvert the angle from radians to an x component
...
def display_goal_dir_on_frame(actual_frame, head_loc, bod_shelt_dir, hdir_barrier):
    ...
    heading_dir_x = int(magnitudeOfVector * np.cos(bs))  # self.bod_shelt_dir  # Convert the angle from radians to an x component
    ...
    if np.any(hdir_barrier):  # bod_barr_dir
        ...
        for i in np.arange(2):  # assuming two edges in barrier (we're not plotting the arrow to the center of the barrier)
            ...
            if hdir_barrier[i] > 0:
                bs = hdir_barrier[i] - np.pi
            heading_dir_x = int(magnitudeOfVector * np.cos(bs))  # bod_barr_dir  # Convert the angle from radians to an x component
            heading_dir_y = -int(magnitudeOfVector * np.sin(bs))  # Convert the angle from radians to an y component
            # Plot the heading direction on the frame centered at the animal's average location
            cv2.arrowedLine(
                actual_frame,
...
def display_all_trail_segments(actual_frame, trail, trail_colors, trail_thicknesses):
    for j, (line, line_color, line_thickness) in enumerate(zip(trail, trail_colors, trail_thicknesses)):
        if j:
            cv2.line(
                actual_frame,
                line,
