...
def plot_the_circular_rho(video_df, save_path) -> None:
    ...
    # INIITIALIZE
    columns = select_angle_columns(video_df).columns
    perms = create_all_the_permutations_of_angles(columns)

    # Create optimal rho dict
    optimals = load_optimals(save_path)
    optimal_rho_dic = loop_through_permutations_of_angles_and_apply_circcoeff(perms, optimals)
    optimal_rhos = list(optimal_rho_dic.values())
    x_labels = [f"{perms[i][0]} VS {perms[i][1]}" for i in range(len(perms))]

    # Create real rho dict
    rho_dic = loop_through_permutations_of_angles_and_apply_circcoeff(
        perms, video_df.filter(pl.col("OutofshelterIdx") == True)
    )
    ...
    plt.bar(x - 0.1, real_rhos, color="darkgrey", label="Real data", width=0.2)
    plt.bar(x + 0.1, optimal_rhos, color="lightgreen", label="Optimal data", width=0.2)
    ax.set_xticks(x, x_labels)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.spines["left"].set_visible(False)

    # NOTE = used twice now, re
    ...
    plt.ylabel("Circular correlation coefficient", fontsize=20)
...
def compute_the_circular_rho(behaviour_df) -> dict:
    """computes the circular correlation coefficient between angles

    Ingests the frame by frame behavioural data, filters on the
    available angles when the mouse is out of the shelter and then
    computes coeff"""
    angles = select_angle_columns(behaviour_df)
    combos = create_all_the_permutations_of_angles(angles.columns)
    rho_dic = loop_through_permutations_of_angles_and_apply_circcoeff(combos, angles)
    return rho_dic
...
# NOTE - This should be a shared componnet somewhere
def select_angle_columns(videoDf) -> pl.DataFrame:
    """Help filter if mushroom or barrier experiment"""
...
def loop_through_permutations_of_angles_and_apply_circcoeff(combinations, video_df) -> dict:
