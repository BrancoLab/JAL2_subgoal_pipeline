...
class Process:
    ...
    @staticmethod
    def _registration_metadata_name() -> str:
        return "registration_data.json"

    def _registration_metadata_path(self, session: NEW_Session = None) -> str:
        if session is None:
            session = self.session
        return os.path.join(session.base_path, session.processed_path, self._registration_metadata_name())

    @staticmethod
    def _session_metadata_name() -> str:
        return "metadata.json"

    def _session_metadata_path(self, session: NEW_Session = None) -> str:
        if session is None:
            session = self.session
        return os.path.join(session.base_path, session.processed_path, self._session_metadata_name())
    ...
    def create_session(self, settings) -> NEW_Session:
        ...
        if not settings.create_new_registration:
            self.loaded_registration_transform, shelter_location, barrier_location = self.load_registration_transform()
            self.session.shelter_location = shelter_location
            self.session.barrier_location = barrier_location
        else:
        ...
        self.save_registration_transform(registration_transform)
    ...
    def save_session(self, overwrite=True) -> None:
        ...
        """
        meta_file = self._session_metadata_path(self.session)
        assert not os.path.isfile(meta_file) or overwrite, "Permission to save not granted"
        session_dict = self._json_compatible(self._serialize_session_payload(self.session))
    ...
    def load_session(self) -> NEW_Session:
        ...
        try:
            # set up session
            self.session.base_path, _ = get_computer_specific_paths(self.session.file_path, return_ceph=True)

            # if session dict json does not exist,load legacy pickled metadata and migrate to JSON
            meta_json_path = self._session_metadata_path(self.session)
            if not os.path.isfile(meta_json_path):
                # Legacy pickle migration path.
                legacy_meta_path = os.path.join(self.session.base_path, self.session.metadata_file)
                ...
                self._apply_session_payload(self.session, legacy_payload)
                ...
                if isinstance(legacy_transform, np.ndarray):
                    self.save_registration_transform(legacy_transform, session=self.session)

                self.save_session(overwrite=True)
    ...
    def save_registration_transform(self, registration_transform: object, session: NEW_Session = None) -> None:
        ...
        os.makedirs(os.path.dirname(self._registration_metadata_path(session)), exist_ok=True)
        with open(self._registration_metadata_path(session), "w", encoding="utf-8") as f:
            json.dump(self._json_compatible(payload), f)

    def load_registration_transform(self, session: NEW_Session = None):
        """
        ...
        registration_path = self._registration_metadata_path(session)
        if not os.path.isfile(registration_path):
            # Backward compatibility: try loading from legacy pickled metadata.
            legacy_metadata_path = os.path.join(session.base_path, session.metadata_file)
            if os.path.isfile(legacy_metadata_path):
                try:
                    ...
                    legacy_transform = getattr(getattr(legacy_session, "video", None), "registration_transform", None)
                    legacy_shelter_location = getattr(legacy_session, "shelter_location", session.shelter_location)
                    legacy_barrier_location = getattr(legacy_session, "barrier_location", session.barrier_location)
            ...
            return None, session.shelter_location, session.barrier_location
        ...
        shelter_location = payload.get("shelter_location", session.shelter_location)
        barrier_location = payload.get("barrier_location", session.barrier_location)
