...
class Verifications:
    ...
    def verify_all_frames_saved(self):
        ...
        """
        if self.Process.camera_trigger.num_frames != self.Process.session.video.num_frames:
            ...
            logger.warning(
                f"The number of camera triggers ({self.Process.camera_trigger.num_frames}) does not match the number of video frames ({self.Process.session.video.num_frames})"
            )
    ...
    def verify_aligned_data_streams(self):
        ...
        if self.Process.camera_trigger.num_samples != self.Process.session.audio.num_samples:
            logger.error(
                f"The number of samples in the audio {self.Process.session.audio.num_samples} and camera trigger {self.Process.camera_trigger.num_samples} streams do not match after alignment"
            )
            assert (
                self.Process.camera_trigger.num_samples == self.Process.session.audio.num_samples
            ), "The length of camera trigger doesn't match the length of the audio, processing has failed, kill script"
    ...
    def verify_onsets_and_offsets(self):
        ...
        if len(bonsai_sync_onsets) != len(ephys_sync_onsets):
            ...
            logger.warning("When inspecting probe sync signal found different number of pulses for bonsai: " f"{len(bonsai_sync_onsets)} and SpikeGLX: {len(ephys_sync_onsets)}")
    ...
    def visulize_sync_output(self) -> tuple:
        ...
        # Align signals
        slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(self.Process.ttl.ephys_sync_onsets, self.Process.ttl.bonsai_sync_onsets)

        regression = lambda x: (slope * x) + intercept

        plt.scatter(self.Process.ttl.bonsai_sync_onsets / self.Process.ttl.sampling_rate, self.Process.ttl.ephys_sync_onsets / self.Process.ttl.sampling_rate)
        ...
        axs[1].plot(
            regression(imec_samples)[middlePulseEfizz : middlePulseEfizz + 500000] / self.Process.ttl.sampling_rate,
            imecSignal[middlePulseEfizz : middlePulseEfizz + 500000],
            color="blue",
            label="Imec",
        )
        axs[1].plot(bonsai_samples[middleBon : middleBon + 500000] / self.Process.ttl.sampling_rate, bonsaiSignal[middleBon : middleBon + 500000], color="red", label="Bonsai")
        ...
        axs[2].plot(
            regression(imec_samples)[LastPulsesEfizz : LastPulsesEfizz + 500000] / self.Process.ttl.sampling_rate,
            imecSignal[LastPulsesEfizz : LastPulsesEfizz + 500000],
            color="blue",
            label="Imec",
        )
        axs[2].plot(
            bonsai_samples[LastPulsesBon : LastPulsesBon + 500000] / self.Process.ttl.sampling_rate,
            bonsaiSignal[LastPulsesBon : LastPulsesBon + 500000],
            color="red",
            label="Bonsai",
        )
