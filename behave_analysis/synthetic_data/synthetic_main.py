"""A module to generate synthetic cells and spikes tuned to task relevant features in order to test analysis methods"""
...
def generate_synthetic_dataframe(
    tuning: list, realistic=True, num_cells_per_type=37, number_of_spikes_to_gen_per_cluster=250000, pass_video_df=None
) -> pl.DataFrame:
    """
    Generate a synthetic polars dataframe that attempts to match the real data.

    Args:
        tuning (list): A list of strings. Each entry contains an angle and can range from a single angle ['hdir] to 4. E.g: ['hdir', 'hsa', 'h_preflipbar_a', 'h_postflipbar_a']
        ...
        num_cells_per_type (int): How many different cluusters do you want per tuning category, the more cells the more angles that will be covered / generated for

    Returns:
        synth_df (type: pl.DataFrame)
    """
    ...
    for cell_type in tuning:
        print(f"Generating sythentic {cell_type} cells")
        angles = extract_tuning_request(cell_type_to_generate=cell_type, tracking_data=tracking)
        spikes, clusters = return_spike_times_locked_to_behavioural_direction(
            behavioural_direction=angles.to_numpy(),
            number_of_spikes=number_of_spikes_to_gen_per_cluster,
            tuned_direction=np.linspace(-np.pi, np.pi, num_cells_per_type),
            dir_std=np.random.choice(np.linspace(0.1, np.pi / 4, 50), size=num_cells_per_type),
            fps=40,
            mean=0,
            std_dev=1,
            add_noise=False,
    ...
    synth_df = pl.DataFrame(
        {
            "spike_times": [0] * len(all_spikes),  # Not used but kept to match production data
...
def return_spike_times_locked_to_behavioural_direction(
    behavioural_direction, number_of_spikes, tuned_direction, dir_std=0.1, fps=40, mean=0, std_dev=1, add_noise=False, noise_scale=0.1
    ...
    for idx, items in enumerate(zip(tuned_direction, number_of_spikes, dir_std)):
        ...
        indices = np.where(
            np.logical_and((behavioural_direction >= (tuned_direction - dir_std)), (behavioural_direction <= (tuned_direction + dir_std)))
        )[0]
        times = indices / fps
...
def efizz_stats(session):
    ...
    spikedataframe = spike_data.filter((spike_data["cluster_group"] == "good") | (spike_data["cluster_group"] == "mua"))
    spikecount = spikedataframe.group_by("spike_clusters").count()
    number_of_spikes = spikecount["count"].to_numpy()
    return number_of_spikes


def generate_spikes(spike_count, times, mean, std_dev) -> np.ndarray:
    """
