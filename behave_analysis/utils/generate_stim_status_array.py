...
def generate_stim_status_array(onset_frames, stimulus_durations, seconds_before, seconds_after, fps) -> np.ndarray:
    ...
    stim_status[int(seconds_before*fps+onset_frames-int(onset_frames)):int((seconds_before+stimulus_durations)*fps)+onset_frames-int(onset_frames)]=0 # 0 ~ stimulus is ON
