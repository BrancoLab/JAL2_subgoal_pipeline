...
def load_or_open(path_file_location: str, data_type: str, **kwargs):
    ...
    logger.info("File will be saved or opened at: {}".format(savepath))
    if savepath.exists():
        logger.info("Loading a previously converted bin file from a npy file")
        return np.load(savepath)
    else:
        logger.info("Npy file does not exist. Opening a binary and saving to numpy to save time on opening")
        ...
        logger.info("Npy file created - check directory")
        return signal