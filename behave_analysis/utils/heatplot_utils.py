...
def add_features_binned(ax, condition: str, tracking: dict, xbins: np.array, ybins: np.array) -> None:
...
def add_features(ax, condition: str, tracking: dict, zero_centre=False) -> None:
    """Draw arena and barrier on top of heatmaps or scatterplots
    ...
    # draw shelter
    if "shelter_loc" in tracking.keys():
        shelt = tracking["shelter_loc"]
        if zero_centre:
            shelt = [[element - 512 for element in sublist] for sublist in shelt]
        for i in [0, 1]:
            ax.plot([shelt[0][0], shelt[1][0]], [shelt[i][1], shelt[i][1]], color="k")
            ax.plot([shelt[i][0], shelt[i][0]], [shelt[0][1], shelt[1][1]], color="k")
    ...
    if not np.logical_or(condition == "shelter_only", condition == "pre_shelter"):
        if len(tracking["barrier_loc"]) > 0:
            ...
            if condition != "barrier_removed":
                bar_y = [tracking["barrier_loc"][0][1], tracking["barrier_loc"][1][1]]
                if zero_centre:
