...
def draw_significance_bar(ax, x1, x2, y, text, height):
    ax.plot([x1, x1, x2, x2], [y, y + height, y + height, y], color="black", linewidth=1.0)
    ax.text((x1 + x2) / 2, y + height, text, ha="center", va="bottom", fontsize="small")
...
def plot_delta_by_mouse(pa_dict, time_window, conditions, save_path=None):
    ...
    for color, (mouse, session_list) in zip(colors, mice_groups.items()):
        ...
        mouse_mean_deltas.append(mean_deltas)
        ax.plot(
            x,
            mean_deltas,
            marker="o",
    ...
    if mouse_mean_deltas:
        mouse_mean_deltas_arr = np.vstack(mouse_mean_deltas)
        mean_across_mice = np.nanmean(mouse_mean_deltas_arr, axis=0)
        ax.bar(
            x,
            mean_across_mice,
            width=0.5,
            color="gray",
            alpha=0.3,
            label="Across mice mean",
            zorder=0,
        )

        condition_indices = {cond: idx for idx, cond in enumerate(conditions)}
        comparison_pairs = [
            ("shelter_only", "barrier_pre_flip"),
            ("shelter_only", "barrier_post_flip"),
            ("barrier_pre_flip", "barrier_post_flip"),
        ]
        stats_results = []
        for cond_a, cond_b in comparison_pairs:
            idx_a = condition_indices.get(cond_a)
            idx_b = condition_indices.get(cond_b)
            if idx_a is None or idx_b is None:
                continue
            data_a = mouse_mean_deltas_arr[:, idx_a]
            data_b = mouse_mean_deltas_arr[:, idx_b]
            mask = np.isfinite(data_a) & np.isfinite(data_b)
            if mask.sum() < 2:
                continue
            try:
                _, p_value = wilcoxon(data_a[mask], data_b[mask], alternative="two-sided")
            except ValueError:
                p_value = 1.0
            stats_results.append((idx_a, idx_b, p_value))
        ...
        if stats_results:
            ...
            step = 0.08 * span
            bar_height = 0.02 * span
            for offset, (idx_a, idx_b, p_val) in enumerate(stats_results):
                y = base + offset * step
            ...
            ax.set_ylim(y_min, base + len(stats_results) * step + 0.1 * span)
    ...
    fig.tight_layout()
    if save_path:
        path = save_path.replace(".png", "_per_mouse.eps")
