...
cluster 17 in jal004 flipppuf19sept_2023_09_19T14_10_56 coloured by head direction (hdir)"""
...
ANGLE_BIN_EDGES = np.linspace(0, 360, 361)
...
def digitize_head_direction(hdir_radians):
    ...
    degrees = np.mod(np.rad2deg(np.asarray(hdir_radians)), 360)
...
def spatial_position_firing_hdir(video_df, save_path):
    ...
    video_df = video_df.filter(((video_df["mouse_x_position"] - arena_center_x) ** 2 + (video_df["mouse_y_position"] - arena_center_y) ** 2) <= (arena_radius**2))

    plot_ax.scatter(
        video_df["mouse_x_position"].to_numpy(), video_df["mouse_y_position"].to_numpy(), s=7, c="grey", linewidths=0, marker=".", alpha=0.01, rasterized=True
    ...
    spike_filt = spike_filt.filter(((spike_filt["mouse_x_position"] - arena_center_x) ** 2 + (spike_filt["mouse_y_position"] - arena_center_y) ** 2) <= (arena_radius**2))

    hdir = digitize_head_direction(spike_filt["hdir"])
    cc = hsv_hdir_colormap(hdir)
    plot_ax.scatter(
        spike_filt["mouse_x_position"].to_numpy(), spike_filt["mouse_y_position"].to_numpy(), s=7, c=cc, linewidths=0, marker=".", rasterized=True
