...
def compress_vars(var, neural_matrix):
    ...
    for i, neural_activity in enumerate(neural_matrix):
        ...
        change_points = np.where(np.diff(var) != 0)[0] + 1  # Indices where position changes
        change_points = np.insert(change_points, 0, 0)  # Include the start of the first segment
        change_points = np.append(change_points, len(var))  # Include the end of the last segment

        # Step 2: Compress position and neural activity
        compressed_pos = [var[start] for start in change_points[:-1]]
        compressed_activity = [neural_activity[start:end].mean() for start, end in zip(change_points[:-1], change_points[1:])]  # Example: mean activity for each segment
    ...
    for i in range(nbins):
        ...
        if np.any(mask):  # Check if the bin has any data
            ...
            if len(non_nan_arr) > 0:
                angles_firing[i] = np.mean(mstats.winsorize(non_nan_arr, limits=(0.15, 0.15)))
    if remove_empty:
        angles_firing = angles_firing[~np.isnan(angles_firing)]  # Remove empty bins
