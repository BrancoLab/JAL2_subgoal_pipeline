    ...
    for i, con in enumerate(conditions):
        ...
        for idx, (onset, offset) in enumerate(zip(onsets, offsets)):
            ...
            if np.logical_or(con == trial_condition, con == "all_time"):
                ...
                if tracking_data["bod_shelt_dir"][start_frame] < 0:
                    bsa = tracking_data["bod_shelt_dir"][start_frame] + np.pi
                if tracking_data["bod_shelt_dir"][start_frame] > 0:
                    bsa = tracking_data["bod_shelt_dir"][start_frame] - np.pi

                if np.logical_and(exclude_south, np.logical_or(con == "barrier_pre_flip", con == "barrier_post_flip")):
                ...
                if np.logical_and(tracking_data["hdir"][onset - 1] > good_starting_angle[0], tracking_data["hdir"][onset - 1] < good_starting_angle[1]):
