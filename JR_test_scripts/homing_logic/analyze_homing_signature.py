...
class PhaseOneAnalyzer:
    ...
    def __init__(self, experiments_dict: Dict, output_dir: str = None):
        ...
        self.output_dir = Path(output_dir) if output_dir else Path("behave_analysis/analyze/homing_analysis_results")
    ...
    def create_frame_labels(self):
        ...
        for session_name, data in self.session_data.items():
            ...
            logger.info(f"  {session_name}: {np.sum(is_target)} target frames, " f"{np.sum(is_exploration)} exploration frames")
    ...
    def validate_run_hypothesis(self) -> Dict:
        ...
        for session_name, data in self.session_data.items():
            ...
            logger.info(f"  {session_name}: {pct_in_runs:.1f}% of target in runs " f"(hypothesis={'✓' if hypothesis_confirmed else '✗'})")
            ...
            for run_idx, (onset, offset) in enumerate(runs):
                try:
                    ...
                    if px_per_cm is None or px_per_cm == 0:
                        px_per_cm = 1.0
                    features = self._extract_run_features(run_idx, session_name, onset, offset, tracking_data, video_df, is_target, fps, px_per_cm)
    ...
    def _extract_run_features(self, run_idx, session_name, onset, offset, tracking_data, video_df, is_target, fps, pixel_per_cm) -> RunFeatures:
    ...
    def compute_feature_distributions(self):
        ...
        feature_names = [
            "speed_mean",
            "speed_peak",
            "mean_initial_acceleration",
            "head_turn_angle_initial",
            "initial_hdir_change_abs",
            "mean_initial_angular_head_velocity",
            "net_dx",
            "net_dy",
            "net_distance",
            "displacement_vertical_ratio",
            "run_duration",
        ]
        ...
        for fname in feature_names:
            ...
            cohens_d = (target_mean - exploration_mean) / np.sqrt((target_std**2 + exploration_std**2) / 2) if (target_std**2 + exploration_std**2) > 0 else 0
    ...
    def plot_feature_distributions(self, only_top_n: int = 6):
        ...
        for idx, (fname, stats_dict) in enumerate(self.feature_ranking[:n_plots]):
            ...
            if fname == "speed_peak":
                ax.set_xlim(0, 150)
            if fname == "mean_initial_acceleration":
                ax.set_xlim(0, 50)
            if fname == "displacement_vertical_ratio":
                ax.set_xlim(-1, 1)
            if fname == "run_duration":
                ax.set_xlim(0, 50)
            if fname == "net_dx":
                ax.set_xlim(-75, 75)
            if fname == "head_turn_angle_initial":
                ax.set_xlim(-np.pi, np.pi)
            if fname == "initial_hdir_change_abs":
                ax.set_xlim(0, np.pi)
            if fname == "mean_initial_angular_head_velocity":
                ax.set_xlim(0, 5)

        # Add legend (shared across all subplots)
        from matplotlib.lines import Line2D

        legend_elements = [Line2D([0], [0], color="blue", lw=2.5, label="Target (n sessions)"), Line2D([0], [0], color="orange", lw=2.5, label="Exploration (n sessions)")]
    ...
    def generate_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write("-" * 80 + "\n")
            all_confirmed = all(v["hypothesis_confirmed"] for v in self.hypothesis_results.values() if v["hypothesis_confirmed"] is not None)
            ...
            f.write("-" * 80 + "\n")
            f.write("{:<6} {:<35} {:<12} {:<8} {:<12}\n".format("Rank", "Feature", "Cohen's d", "AUC", "p-value"))
            f.write("-" * 80 + "\n")

            for idx, (fname, stats_dict) in enumerate(self.feature_ranking[:15]):
                f.write(f"{idx+1:<6} {fname:<35} {stats_dict['cohens_d']:>11.3f} " f"{stats_dict['auc']:>7.3f} {stats_dict['pval']:>11.3e}\n")
    ...
    def run_phase2_classification(
        ...
        for fname in selected:
            g = gates[fname]
            logger.info(f"  gate {fname}: keep if value {g['direction']} {g['threshold']:.4f} " f"(feature recall={g['recall']:.3f}, precision={g['precision']:.3f})")
    ...
    def compute_phase3_fractional_overlap(
        ...
        for session_name, data in self.session_data.items():
            ...
            strict_pair_precision = (
                float(np.mean((np.array(candidate_best_manual_frac) >= manual_fraction_threshold) & (np.array(candidate_best_candidate_frac) >= candidate_fraction_threshold)))
                if len(candidate_best_manual_frac) > 0
                else 0.0
            )
