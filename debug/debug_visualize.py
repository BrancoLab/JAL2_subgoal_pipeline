...
def visualize():
    ...
    for session_ID in experiments_objects:
        ...
        if settings_v.laser_trials:
            Visualize(session, settings_v).trials(stim_type="laser")
        if settings_v.escape_trials:
            Visualize(session, settings_v).trials(stim_type="audio")
        if settings_v.homing_trials:
            Visualize(session, settings_v).trials(stim_type="homing")
        if settings_v.t_xing_trials:
            Visualize(session, settings_v).trials(stim_type="threshold_crossing")
